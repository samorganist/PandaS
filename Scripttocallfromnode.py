...
tmps1=time.clock()
import sys,json
sys.path.append('C:\Users\Samorg\Desktop\TestNode\SmartR\FinalScripts\PandaS')
sys.path.append('C:\Users\Samorg\Desktop\TestNode\SmartR\FinalScripts')
...
filename = "Rapport_%s"%x[0]["%s"%pp]["user"]["firstname"]+"_%s.pdf"%idd
...
server = smtplib.SMTP('smtp.pandaorthopedics.com', 587)
server.starttls()
...
def squatAnalyse():
    ...
    minn=detect_peaks(filtreflex,mph=-60,mpd=20,edge='rising',valley='true')
    k=0
    maxxx=[]
    minnn=[]  
#################################################################################################
...
#del c
    ...
    if "comment" in x[0]:
        c.drawString(20,650," Commentaire:%s"%x[0]["comment"])
    else :
...
#idd=ObjectId(ide)
#print(ide)
idd=ObjectId("5989d5bafa1b5ae2bfc152fc")
